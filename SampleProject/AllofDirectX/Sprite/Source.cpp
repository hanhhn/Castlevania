...
void Render()
{
	srect.left = (Index % SpritePerRow)*(BaseWidth);
	srect.top = (Index / SpritePerRow)*(BaseHeight);
	srect.right = srect.left + BaseWidth;
	srect.bottom = srect.top + BaseHeight;
}
/*------------------*/
...
int GameInit(HWND hwnd)
{
	...
	//Texture
	texture = LoadTexture("simon.png", D3DCOLOR_XRGB(0,64,128));
...
void GameRun(HWND hwnd)
{
	...
	if (d3ddv->BeginScene())
	{
		...
		//RECT srect = simon->Render();
		
		Next();
		Render();

		SpriteHandler->Begin(D3DXSPRITE_ALPHABLEND);
		D3DXVECTOR3 position_in((float)100, (float)100, 0);
